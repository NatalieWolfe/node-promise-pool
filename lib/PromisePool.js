...
function PromisePool(opts) {
    ...
    if (opts.log instanceof Function) {
        this._log = logger.fromFunction(this._opts.name, opts.log);
    }
    else if (opts.log === true) {
        this._log = logger.makeConsole(this._opts.name);
    }
    else if (opts.log && opts.log.info instanceof Function) {
        this._log = opts.log;
    }
    else {
        this._log = logger.noop;
    }

    this._availableObjects = [];
    this._waitingClients = new PriorityQueue(opts.priorityRange || 1, this._log);
    this._count = 0;
...
/**
 * @typedef {object} PromisePool.Factory.Logger
 *
 * A bunyan-style logging object with the following log methods:
 *
 *  - trace
 *  - debug
 *  - info
 ...
 *
 * @property {bool|PromisePool.Factory.log|PromisePool.Factory.Logger} log
 *  Optional. Whether the pool should log activity. If a function is provided, it will be called to
 *  log messages. If `true` is provided, messages are logged to `console.log`. If a bunyan-style
 *  logging object is provided, that will be used. Defaults to `false`.
 *
...
function _ensureMinimum() {
    ...
    this._log.trace(
        '_ensureMinimum() - Acquiring connections to reach minimum - min=%d diff=%d',
        this._opts.min, diff
    );
...
function _createResource() {
    this._log.trace(
        '_createResource() - creating client - count=%d min=%d max=%d',
 ...
 * @this {PromisePool}
 *
 * @return {Promise} A promise for when all idle connections have been removed.
 */
function _removeIdle() {
    ...
    for (
        ...
        if (now >= timeout) {
            ...
            this._log.trace('removeIdle() - destroying obj - now:%d timeout:%d', now, timeout);
    ...
    if (this._availableObjects.length > 0) {
        this._log.trace('availableObjects.length=%d', this._availableObjects.length);
        _scheduleRemoveIdle.call(this);
    }
    else {
        this._log.trace('removeIdle() - all objects removed');
    }
...
function _dispense() {
    ...
    this._log.info(
        '_dispense() - waiting=%d available=%d',
        waitingCount, this._availableObjects.length
    );

    if (waitingCount > 0) {
        while (this._availableObjects.length > 0) {
            this._log.trace('_dispense() - reusing obj');
        ...
        if (this._count < this._opts.max) {
            ...
            _createResource.call(this).then(function(client){
                ...
                --self._count;
                self._log.error({error: err}, '_dispense() - failed to create resource!');
                var waiter = self._waitingClients.dequeue();
                if (waiter) {
                    waiter.reject(err);
                }
                    ...
                    self._log.warn('_dispense() - No clients to send resource creation error.');
                    throw err;
                }
            });
        }
...
PromisePool.prototype.release = function(obj) {
    ...
    if (this._availableObjects.some(_objFilter(obj, true))) {
        this._log.error('release called twice for the same resource: %s', (new Error().stack));
        return;
    }

    if (obj.__promisePool_destroyed) {
        this._log.info('Released resource is destroyed, not returning to pool.');
    }
    else {
        var self = this;
        var _release = function(){
            ...
            self._log.trace('timeout: %d', objWithTimeout.timeout);
        };
        ...
        if (this._opts.onRelease) {
            Promise.resolve(this._opts.onRelease(obj)).then(function(){
                ...
                self._log.error({error: err}, 'Error releasing client!');
                self.destroy(obj);
            });
        }
        else {
 ...
 *
 * @param {PromisePool.Client} obj - The acquired item to be destoyed.
 *
 * @return {undefined}
 */
PromisePool.prototype.destroy = function(obj) {
    this._log.trace('Destroying object, count=%d', this._count);
    --this._count;
    ...
    return new Promise(function(resolve, reject){
        function check(){
            if (self._waitingClients.length > 0) {
                ...
                self._log.trace('Delaying drain, %d clients in queue.', self._waitingClients.length);
                ...
                self._log.trace('Delaying drain, %d items need to be released.', missingCount);
                setTimeout(check, self._opts.drainCheckInterval);
            }
            else {
...
PromisePool.prototype.destroyAllNow = function() {
    this._log.info('force destroying all objects');
...
Object.defineProperty(PromisePool.prototype, 'max', {
    ...
    set: function(max){
        if (typeof max != 'number') {
            throw new Error('Pool `max` must be an integer.');
        }
        this._opts.max = Math.floor(Math.max(max, this.min));
        return this.max;
    },
    enumerable: true
});
...
Object.defineProperty(PromisePool.prototype, 'min', {
    ...
    set: function(min){
        if (typeof min != 'number') {
            throw new Error('Pool `min` must be an integer.');
        }
        var oldMin = this.min;
        this._opts.min = Math.floor(Math.min(min, this.max));
